...
# It is important to remember that sometimes, .csv or .txt files can be saved 
# using different regional settings. By default, the read.csv() command expects
# a comma ',' to separate data/observations in a file. However, given the regional
# setting, a semicolon, ';' may have been used. For this reason, we either need 
# to specify the separator, like above (sep = ";"), or we can use the command
# read.csv2().
personality2 <- read.csv2("personalidad.csv")

# If we do not do this, we will not get an error, but we will have a file where 
# the data is not correctly organized.
personality_wrong <- read.csv("personalidad.csv")

# Run these lines of code and compare. You can also see differences in the data
# in the Environment window. 
View(personality)
View(personality2)
View(personality_wrong)

# Now, we can remove the data frames we won't use
rm(personality2, personality_wrong)
...
# click on the broom that appears in the "Workspace Pane" (to your right). The 
# Environment tab must be selected.
...
# Binning
personality$OpenessGroup <- cut(personality$Openness, 
                                breaks = 3, 
